...
void BM_udp_pingpong(benchmark::State& state, std::string mode, std::string nic, int port, std::string group_ip, int size){
    ...
    while (state.KeepRunning())
    {
        ...
        ssize_t recv_size = recvfrom(sd, data, sizeof(data), 0, (struct sockaddr *)&groupSock, &groupSock_len);
        if(recv_size != size){
            perror("recvfrom err!");
            exit(1);
        }
...
BENCHMARK_CAPTURE(BM_udp_pingpong, pingpong, "-client", "127.0.0.1", 50000, "224.0.0.1", 1024);
BENCHMARK_MAIN();
...
{
    if(argc <= 2){
        std::cerr << "Usage: " << argv[0] << " <msg type> <count>" << std::endl;
        std::cerr << "Example: " << argv[0] << " SecurityDefinition 10000" << std::endl;
        return 1;
    }
    ...
    auto count = atol(argv[2]);
    char target[2046];
    int  size = 0;
    if(type == "SecurityDefinition") {
    ...
    {
        auto now = clock.now();
        processMsg<Processor>(target, size - 1);
        l.add(clock.now() - now);
    }
    l.generate(std::cout);
}